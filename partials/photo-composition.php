<div class="photo-composition">
  <div class="photo-composition__photo">
    <img src="<?php echo get_images_path() . 'skateboard.jpg'; ?>" alt="">
    <h2>Creation + Strategy + Amplification</h2>
  </div>
  <div class="photo-composition__content">
    <h3>We take a holistic approach to branding and promotion by taking on each brand from inception to digital transformation.</h3>
    <p>From strategy and planning right through to website creation, design innovation, social media and content amplification, we’ve got it covered.</p>
    <a href="#" class="btn btn--hollow">learn more</a>
  </div>
</div>
<div class="photo-composition">
  <div class="photo-composition__content">
    <h3>We’re experts in engaging audiences that matter, and converting clicks to conversations.</h3>
    <p>Whether you’re a start-up looking for a place to build it all, or an existing business looking for a refresh, our team of content, design and social media specialists provide world-class marketing solutions customised to suit your needs</p>
    <a href="#" class="btn btn--hollow">learn more</a>
  </div>
  <div class="photo-composition__photo">
    <img src="<?php echo get_images_path() . 'biking.jpg'; ?>" alt="">
    <h2>Global approach + Local appeal</h2>
  </div>
</div>
